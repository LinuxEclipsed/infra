resource "proxmox_vm_qemu" "k3s" {
    ...
    provisioner "local-exec" {
      command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u root -i '${self.default_ipv4_address},' -e @../../../ansible/k3s.enc --ask-vault-pass --private-key ${var.pvt_key} -e 'pub_key=${var.pub_key}' ../../../ansible/k3s.yaml"
  }
